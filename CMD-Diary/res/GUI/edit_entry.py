...
def edit_entry():
    ...
    def get_date():
        with open(f"{path}{thisyear.get()}-{thismonth.get()}-{day.get()}.txt", "r") as entry:
            textbox.delete(1.0,END)
            textbox.insert(0.0,entry.read())

    def save_entry():
        with open(f"{path}{thisyear.get()}-{thismonth.get()}-{day.get()}.txt", "w") as entry:
            entry.write(textbox.get(1.0,END))


    save_button = Button(root, text="Save", command=save_entry, padx=20, pady=78)
    save_button.place(x = 670, y = 258)

    get_button = Button(root, text="Get\nDate", command=get_date, padx=20, pady=70)
    get_button.place(x=670,y=50)
